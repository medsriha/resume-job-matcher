...
RESUME_SECTIONS = {
    ...
    'Professional DevelopmentResearch Experience',
    'Professional Experience',
    'Professional GoalsTechnical Skills',
...
class Profile(object):
    ...
    def __get_position_year(self, text) -> list:
        ...
        """
        # This will look for patterns like "Dec 2022," and "January-2022," or "01/2023," "05-2022," and "11.2024"
        pattern = r"(?:(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s\d{4}|\d{1,2}[ /\.-]\d{4})"
    ...
    def __is_section_skills(self, text) -> bool:
        ...
        if text in ("SKILLS", "skills", "Skills", "Qualifications", "QUALIFICATIONS", "Highlights", "HIGHLIGHTS") or\
              re.findall(r"(Technical|Summary|TECHNICAL|SUMMARY)(?: of|OF)? (Proficiencies|Qualifications|Expertise|PROFICIENCIES|QUALIFICATIONS|EXPERTISE)", text):
            return True
        return False
...
"""
# Test snipet
resumes = read_multiple_pdf("resumes//", k = 5)
"""
...
if __name__ == "__main__":
    ...
    path = "C:\\Users\\medSr\\Documents\\resume\\resumes\\MISC\\MohamedSriha-Discord.pdf"
    #path ="C:\\Users\\medSr\\Documents\\resume\\resumes\\DESIGNER\\22675694.pdf"
    resume_txt = TextExtractor(file_path = path).convert_pdf()
    print(Profile(resume=resume_txt, key=path).create()['experience'])
